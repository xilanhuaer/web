...
<template>
  ...
  <el-card>
      ...
      <el-form :inline="true" :model="condition" class="demo-form-inline" ref="formEl">
        ...
        <el-form-item label="所属分组" prop="name">
          <el-select v-model="condition.main_collection_id" class="m-2" placeholder="Select">
            <el-option
              v-for="item in resultMain.data.list"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            />
      ...
      <el-table-column fixed prop="id" label="id" width="60px" align="center">
        <template #default="{ row }">
          <el-link
            :underline="false"
            type="primary"
            @click="($event) => showSub?.initSubIdData(row.id)"
            >{{ row.id }}</el-link
          >
        </template>
      </el-table-column>
    ...
    <el-pagination
      ...
      :total="resultSub.data.total"
      @current-change="($even) => querySub(condition)"
      @size-change="($event) => querySub(condition)"
    />
